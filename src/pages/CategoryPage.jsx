...
const CategoryPage = () => {
  ...
  const subCategories = ['DRINKS', 'FOOD', 'LOST', 'DRINKS', 'FOOD', 'LOST', 'DRINKS', 'FOOD']; // Uppercase for wireframe
  ...
  return (
    ...
    >
      <h1 className="text-4xl font-bold text-gray-800">{categoryTitle}</h1> {/* Dynamic uppercase title */}
      <p className="text-gray-700 text-lg">{description}</p> {/* Exact description */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6"> {/* 4-col grid */}
        {articles.map((article, index) => (
          <ArticleCard key={index} {...article} />
        ))}
      </div>
      <button className="bg-black text-white px-6 py-3 rounded text-sm font-medium block mx-auto hover:bg-gray-800">Load more</button> {/* Black centered button */}
    </motion.div>
  );
};
