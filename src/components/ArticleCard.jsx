...
const ArticleCard = ({ category, title, author, date, summary, isLarge = false, subCategory }) => {
  const handleClick = () => {
    if (window.pendo) {
  ...
  return (
    ...
    >
      ...
      {subCategory && (
        <span className="text-[10px] tracking-wide uppercase text-gray-500 font-bold block mb-2">{subCategory}</span>
      )}
      ...
      {isLarge ? (
        <div>
          ...
          <div className="text-center">
            {category && (
              <span className="block text-[11px] tracking-[0.1em] uppercase text-gray-500 mb-3">{category}</span>
            )}
